import React from "react";
import Button11 from "./components/button11/Button11";
import Button1 from "./components/button1/Button1";
import Button2 from "./components/button2/Button2";
import Button3 from "./components/button3/Button3";
import Button4 from "./components/button4/Button4";
import Button5 from "./components/button5/Button5";
import Button6 from "./components/button6/Button6";
import Button7 from "./components/button7/Button7";
import Button8 from "./components/button8/Button8";
import Button9 from "./components/button9/Button9";
import Button10 from "./components/button10/Button10";
import Button12 from "./components/button12/Button12";
import Button13 from "./components/button13/Button13";
import Button14 from "./components/button14/Button14";
import Button15 from "./components/button15/Button15";
import Button16 from "./components/button16/Button16";
import Button17 from "./components/button17/Button17";
import Button18 from "./components/button18/Button18";

function App() {
  return (
    <div className="text-center">
      <Button11 />
      <Button1 />
      <Button2 />
      <Button3 />
      <Button4 />
      <Button5 />
      <Button6 />
      <Button7 />
      <Button9 />
      <Button8 />
      <Button10 />
      <Button13 />
      <Button15 />
      <Button16 />
      <Button17 />
      <Button18 />
      <div className="flex border">
        <Button12 />
        <Button14 />
      </div>
    </div>
  );
}

export default App;
